package org.josht.starling.foxhole.displayObjects.screens
{
	...
	public class Scale3ImageScreen extends Screen
	{
		...
		private function rightButton_touchHandler(event:TouchEvent):void
		{
			...
			{
				this._image.width = Math.min(this._maxWidth, Math.max(this._image.height, this._minWidth, this._startWidth + touch.globalX - this._startX));
				this.layoutButtons()
			}
		...
		private function bottomButton_touchHandler(event:TouchEvent):void
		{
			...
			{
				this._image.height = Math.min(this._image.width,  this._maxHeight, Math.max(this._minHeight, this._startHeight + touch.globalY - this._startY));
				this.layoutButtons()
			}
			else if(touch.phase == TouchPhase.ENDED)
			{
